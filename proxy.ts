...
export function proxy(request: NextRequest) {
  ...
  if (request.nextUrl.pathname.startsWith('/api/proxy')) {
    const finnhubUrl = process.env.FINNHUB_API_URL || 'https://finnhub.io/api/v1';
    ...
    const targetUrl = new URL(`${path}?${searchParams.toString()}`, finnhubUrl);
    
    console.log(`[Proxy] Proxying ${request.nextUrl.pathname} to Finnhub`);
